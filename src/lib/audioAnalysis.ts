...
export class AudioAnalyzer {
  ...
  private dataArray: Float32Array | null = null; // Changed to Float32 for accuracy
  private frequencyData: Float32Array | null = null; // Changed to Float32 for dB values
  ...
  private readonly VOICE_THRESHOLD_DB = -40; // Stricter threshold
  ...
  private readonly MIN_SNR = 5; // Minimum SNR for voice detection
  /**
  ...
  initialize(stream: MediaStream): void {
    try {
      ...
      this.analyser.fftSize = 2048; // Balanced for real-time
      ...
      // Initialize data arrays - correct sizes
      this.dataArray = new Float32Array(this.analyser.fftSize); // Full FFT size for time domain
      this.frequencyData = new Float32Array(this.analyser.frequencyBinCount); // Half for frequency
  ...
  getAudioFeatures(): AudioFeatures {
    ...
    try {
      // Get time domain data (waveform) - use float for accuracy
      this.analyser.getFloatTimeDomainData(this.dataArray);
     
      // Get frequency domain data (spectrum) - float dB
      ...
      const isVoiceDetected = volumeDB > this.VOICE_THRESHOLD_DB && snr > this.MIN_SNR;
  ...
  /**
   * YIN Algorithm for pitch detection - optimized with variable window and limited tau
   ...
   */
  private detectPitchYIN(buffer: Float32Array): number { // Changed to Float32 input
    const SIZE = buffer.length;
    const sampleRate = this.audioContext?.sampleRate || 48000;
   
    // Optimized search range - limit maxTau for performance (min pitch 80Hz)
    const minPeriod = Math.floor(sampleRate / this.MAX_PITCH);
    const maxPeriod = Math.floor(sampleRate / this.MIN_PITCH); // ~600
    ...
    // Step 1: Calculate difference function (squared difference) - correct variable window
    ...
    for (let tau = 0; tau < searchSize; tau++) {
      ...
      difference[tau] = sum / windowSize; // Normalize by window size
    }
    ...
    for (let tau = 1; tau < searchSize; tau++) {
      ...
      cmndf[tau] = difference[tau] * tau / runningSum; // Corrected formula
    }
   ...
   */
  private calculateRMS(buffer: Float32Array): number { // Float32 input
    let sum = 0;
  ...
  /**
   * Calculate audio energy from frequency spectrum - linear magnitude
   * Measures overall signal intensity
   */
  private calculateEnergy(frequencyData: Float32Array): number { // Float32 dB input
    let sum = 0;
  ...
  /**
   * Calculate Spectral Centroid - using linear magnitudes
   ...
   */
  private calculateZeroCrossingRate(buffer: Float32Array): number { // Float32 input
    let crossings = 0;
    const length = buffer.length;
