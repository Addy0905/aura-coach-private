...
export class AudioAnalyzer {
  ...
  private readonly VOICE_THRESHOLD_DB = -35; // Minimum dB to consider as voice
  ...
  private readonly MIN_ZCR = 0.05; // Minimum ZCR for speech (too low = silence)
  private readonly MAX_ZCR = 0.3; // Maximum ZCR for speech (too high = noise)
  private readonly MIN_ENERGY = 10; // Minimum energy for voice
  ...
  getAudioFeatures(): AudioFeatures {
    ...
    try {
      ...
      // Calculate ZCR and energy for VAD
      const zcr = this.calculateZeroCrossingRate(this.dataArray);
      const energy = this.calculateEnergy(this.frequencyData);

      // Strengthened VAD: multiple conditions to reduce noise sensitivity
      const isVoiceDetected = 
        volumeDB > this.VOICE_THRESHOLD_DB &&
        snr > this.MIN_SNR &&
        zcr > this.MIN_ZCR &&
        zcr < this.MAX_ZCR &&
      ...
      // 6. Calculate clarity score (multi-factor quality metric)
  ...
  private detectPitchYIN(buffer: Uint8Array): number {
    ...
    const threshold = 0.2; // Raised threshold for stricter detection
