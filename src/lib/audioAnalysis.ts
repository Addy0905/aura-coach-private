...
// Updated: Improved YIN with better interpolation, enhanced noise calibration using rolling average,
// added context state handling, better error resilience, and performance optimizations
...
export interface AudioFeatures {
  ...
  pace: number; // Words per minute (calculated externally from transcript)
...
export class AudioAnalyzer {
  ...
  private dataArray: Float32Array | null = null; // Updated to Float32 for precision
  private frequencyData: Float32Array | null = null; // Updated to Float32
  ...
  async initialize(stream: MediaStream): Promise<void> {
    try {
      // Create audio context with fallback sample rate if 48000 not supported
      ...
      // Initialize data arrays with Float32 for better precision
      ...
      throw new Error('Audio initialization failed. Please check microphone permissions and browser support.');
    }
  }
  ...
  getAudioFeatures(): AudioFeatures {
    ...
    try {
      // Get time domain data (waveform) - updated to Float32
      this.analyser.getFloatTimeDomainData(this.dataArray);

      // Get frequency domain data (spectrum) - updated to Float32
      ...
      // If no voice detected, return defaults
      if (!isVoiceDetected) {
        return this.getDefaultFeatures();
      }
      ...
      // 1. Pitch Detection using YIN Algorithm (optimized with better interpolation)
   ...
   * Updated: Improved parabolic interpolation and handling for edge cases
   */
  private detectPitchYIN(buffer: Float32Array): number {
  ...
  private calculateEnergy(frequencyData: Float32Array): number {
    ...
    for (let i = 0; i < length; i++) {
      const normalized = Math.pow(10, frequencyData[i] / 20); // Convert dB back to linear
  ...
  private calculateSpectralCentroid(frequencyData: Float32Array): number {
    ...
    for (let i = 0; i < length; i++) {
      const magnitude = Math.pow(10, frequencyData[i] / 20); // Linear magnitude
   ...
   * Updated: Use rolling average instead of median for smoother calibration
   */
  private calibrateNoiseFloor(volumeDB: number): void {
    ...
    if (this.calibrationSamples.length >= 20) {
      // Use average of lowest 50% to avoid speech contamination
      const sorted = [...this.calibrationSamples].sort((a, b) => a - b);
      const lowHalf = sorted.slice(0, Math.floor(sorted.length / 2));
      this.noiseFloor = lowHalf.reduce((a, b) => a + b, 0) / lowHalf.length;
  ...
  private isValidPitch(pitch: number): boolean {
    return pitch >= this.MIN_PITCH && pitch <= this.MAX_PITCH && !isNaN(pitch);
  }
