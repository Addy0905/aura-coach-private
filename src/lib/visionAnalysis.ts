...
export class VisionAnalyzer {
  ...
  // For optical flow (temporal tracking)
  ...
  async analyzeFrame(videoElement: HTMLVideoElement, timestamp: number): Promise<BodyLanguageMetrics> {
    ...
    try {
      ...
      const [faceResults, poseResults, gestureResults] = await Promise.all([
        Promise.resolve(this.faceLandmarker.detectForVideo(videoElement, timestamp)),
        Promise.resolve(this.poseLandmarker.detectForVideo(videoElement, timestamp)),
        Promise.resolve(this.gestureRecognizer.recognizeForVideo(videoElement, timestamp)),
      ]);
  ...
  private analyzeFace(results: any): FaceAnalysis {
    ...
    if (this.prevFace) {
      ...
      facialMovement = Math.min(100, totalDisplacement * 500);
    }
    this.prevFace = lm;
  ...
  private calcEAR(lm: any[], eye: 'left' | 'right'): number {
    ...
    if (eye === 'left') {
      [p1, p2, p3, p4, p5, p6] = [33, 160, 158, 133, 153, 144].map(i => lm[i]);
    } else {
      [p1, p2, p3, p4, p5, p6] = [362, 385, 387, 263, 373, 380].map(i => lm[i]);
    }
  ...
  private analyzePosture(results: any): PostureAnalysis {
    ...
    const neckAngle = this.calcAngle(
      { x: nose.x, y: nose.y - 0.1, z: nose.z },
      nose,
  ...
  private calcAngle(p1: any, p2: any, p3: any): number {
    ...
    const cosAngle = dot / (mag1 * mag2 + 0.0001);
